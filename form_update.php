...
if (isset($_POST['simpan'])) {
    ...
    $sql = "UPDATE siswa SET nama='$nama', kelas='$kelas', tgl_lahir='$tgl_lahir',
            alamat='$alamat', kota='$kota', jenis_kelamin='$jenis_kelamin',
            hobby='$hobby', ekskul='$ekskul'
            WHERE nis='$nis'";
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nis-display {
            background: #f4f6f7; padding: 9px 12px; border-radius: 5px;
...
<div class="container">
    <div class="form-card">
        ...
        <form method="POST">
            ...
            $hobbiesCheck = explode(', ', $data['hobby']);
            $ekskulsCheck = explode(', ', $data['ekskul']);
            $hobbies = ['Membaca','Olahraga','Menyanyi','Menari','Travelling'];
            $ekskuls = ['Pramuka','Basket','Voli','Band','Seni Tari','PMR','Rohis','Badminton'];
            ?>
            <div class="form-group">
                <label>NIS</label>
                <div class="nis-display"><?php echo htmlspecialchars($data['nis']); ?></div>
            </div>
            <div class="form-group">
                <label>Nama</label>
            ...
            <div class="form-group">
                ...
                <select name="kelas">
                    ...
                    <option value="<?php echo $k; ?>" <?php if ($data['kelas']==$k) echo 'selected'; ?>><?php echo $k; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <div class="row">
                    ...
                    <select name="bulan">
                        ...
                        foreach ($bulan as $k => $b) {
                            $v = str_pad($k+1, 2, '0', STR_PAD_LEFT);
                            $sel = ($v == $tgl[1]) ? 'selected' : '';
                            echo "<option value='$v' $sel>$b</option>";
                        } ?>
                    </select>
                    <select name="tahun">
                        <?php for ($i=2010; $i>=2000; $i--) {
                            $sel = ($i == $tgl[0]) ? 'selected' : '';
            ...
            <div class="form-group">
                <label>Kota</label>
                <input type="text" name="kota" value="<?php echo htmlspecialchars($data['kota']); ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="check-group">
                    ...
                    <label>
                        <input type="checkbox" name="hobby[]" value="<?php echo $h; ?>"
                            <?php if (in_array($h, $hobbiesCheck)) echo 'checked'; ?>>
                        <?php echo $h; ?>
                    </label>
                    <?php endforeach; ?>
                </div>
